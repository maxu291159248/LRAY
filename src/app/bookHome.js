    ...
    pageCallback: async function (index, jq) {
        ...
        for (var i = start; i < end; i++) {
            ...
            content += '<div class="col-sm-6 col-md-3" >'
                + '<div class="thumbnail">'
                + '<a href="library/manager/book.html?id=' + i + '">'
                + '<div style="position: relative;">'
                + '<img id="cover" class="img-cover" src="' + result[6] + '" width="300" alt="资讯封面"/>'
    ...
    pageCallSearchback: async function (index, jq) {
        ...
        for (var i = start; i < end; i++) {
            ...
            content += '<div class="col-sm-6 col-md-3" >'
                ...
                + '<div align="center">'
                + '<button class="btn btn-danger btn-xs" data-toggle="modal" data-target="#modal"'
                + 'onclick="App.set('+ searchList[i][0] +')下架">'
                + '</button>'
                + '</div>'
                + '</div>'
                + '</div>'
    ...
    removeBook: function () {
        book.deployed().then(function (bookInstance) {
          // bookInstance.isBorrowed.call(ReturnId).then(function (result) {
                if (true) {
                    $("#removeBookBtn").html('下架');
                    $("#removeBookBtn").attr("disabled", false);
                    bookInstance.removeBook(ReturnId, {
