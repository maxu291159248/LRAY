App = {
    ...
    pageCallback: async function(index,jq){
        ...
        for(var i=start;i<end;i++){
            console.log(sortList[i][0]);
            var borrowNum = await App._getBorrowedNums(sortList[i][0]);
            var result = await App._getBookBorrowInfo(sortList[i][0]);
            var end = await App._getBookInfo(sortList[i][0]);
            console.log(result[0]);
            //var nameBorrower = web3.eth.accounts[0];
            content+=' <div class="row">'
                +'<div class="col-xs-1" id="rank">'+(i+1)+'</div>'
                +'<div class="col-xs-1" id="nameWriter">'+ result[1] +'</div>'
                + '<div class="col-xs-3" id="nameBOrrower">'+result[0]+'</div>'
                +'<div class="col-xs-1" id="date">'+fmtDate(result[4].toString())+'</div>'
                +'<div class="col-xs-1" id="style">'+result[2]+'</div>'
                +'<div class="col-xs-2" id="ISBN">'+result[3]+'</div>'
                +'<div class="col-xs-1" id="borrowNum">'+ borrowNum +'</div>'
                +'<div class="col-xs-1" id="score">'+end[10]+'</div>'
                +' <div class="col-xs-1">'
                +'<a href="library/manager/book.html?id='+sortList[i][0]+'"><img id="cover" style="width: 50px;height: 50px;" src='+end[6]+'/></a>'
                +'</div>'
                +'</div>'
        }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////
    _getBookBorrowInfo: function (id) {
    return new Promise(function (resolve, reject) {
        book.deployed().then(function (bookInstance) {
            bookInstance.getBorrowerInfo.call(id).then(function (result) {
                resolve(result);
            }).catch(function (err) {
                //alert("内部错误: " + err);
            });
...
$(function () {
    ...
    // 激活导航
    $("#search-menu").addClass("menu-item-active");
