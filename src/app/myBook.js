App = {
    ...
    pageCallback: async function (index, jq) {
        ...
        for (var i = start; i < end; i++) {
            ...
            content += '<div class="col-sm-6 col-md-3" >'
                + '<div class="thumbnail">'
                + '<a href="library/manager/book.html?id=' + booksList[i] + '">'
                + '<div style="position: relative;">'
                + '<img id="cover" class="img-cover" src="' + result[6] + '" width="400" alt="资讯封面"/>'
    ...
    pageCallReturnback: async function (index, jq) {
        ...
        for (var i = start; i < end; i++) {
            ...
            content += '<div class="col-sm-6 col-md-3" >'
                + '<div class="thumbnail">'
                + '<a href="library/book.html?id=' + booksList[i] + '">'
                + '<div style="position: relative;">'
...
/**
  ...
  * @returns {Promise<void>}
  */
    getPublishedBooksNum: async function () {
        var result = await App._getPublishedBooks();
        window.booksList = result;
        window.totalBooksNum = result.length;
        return window.totalBooksNum;
    },
     ...
     * @private
     */
    _geOffBooks: function () {
        return new Promise(function (resolve, reject) {
            book.deployed().then(function (bookInstance) {
                bookInstance.getOffBooks.call().then(function (result) {
                    result[9] = 0;
                    resolve(result);
                }).catch(function (err) {
                    alert("内部错误: " + err);
